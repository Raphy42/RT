...
static t_vec3       retrace(const t_ray *r, t_scene *scene)
{
    t_vec3          color;

    color = rt_color(r, scene);
    vec3_mul_f(&color, &color, 0.5f);
    return (color);
}
...
t_vec3 rt_color(const t_ray *r, t_scene *scene)
{
    ...
    if (scene_raytrace(scene, r, precision, &record))
    {
        ...
        vec3_add(&target, vec3_add(&target, &record.normal, &record.pos), &random);
        ray_assign(&sub_ray, &record.pos, vec3_sub(&tmp, &target, &record.pos));

        return (retrace(&sub_ray, scene));
    }
    else {
        t_vec3  u;
        float   t;
